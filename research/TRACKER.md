...
> **`F0-A6-BORDE-0001`** (`b921dff`). Interpretación: ADDENDUM III y IV del memo.
> 🔴 **DIVERGENCIA DE NETO = 17,27 %** sobre las 135 emparejadas evaluables (real −12.522,97 USD ·
> réplica −10.360,21 USD). Umbral aceptable **0,3 %** ⇒ **58 veces el umbral**; el ideal de 0,15 %
> está a 115. Todo sin excluir (152 vs 157): **−31,79 %**. Las 147 matched: **−22,99 %**.
> ✅ **Control de la fórmula limpio:** tasa implícita peso/dólar p50 **924,98**, dispersión relativa
> global 0,0093 y **1,06 × 10⁻⁸ dentro de un día** (2026-08-02). El instrumento es de fiar.
> 🔴 **EL SESGO ES DIFERENCIAL, Y ÉSTE ES EL HALLAZGO QUE MANDA:** la réplica **penaliza a S6**
> (−831,47 USD · **−10,39 por posición**) y **favorece a SuperTrend** (+2.994,23 USD · **+54,44 por
> posición**), signos opuestos y cociente **5,24×**. ⇒ **Queda RETIRADA la tranquilidad que el
> controlador dio al user** («si el error es común a las dos, la comparación sobrevive»): no es
> común, y embellece justo a la que en la realidad perdió.
> 🔴 **DATO AJENO A LA PARIDAD Y DE CONSECUENCIA PROPIA:** en las **135 posiciones de puro motor el
> neto real es NEGATIVO: −12.522,97 USD**. La cuenta termina en **+17.671,25 USD** únicamente por
> los **11 cierres manuales del operador (+30.965,39 USD)** más el TP (+1.249,55). Reconciliación
> verificada por el controlador: −12.522,97 + 32.214,94 = +19.691,97 (147) − 2.020,72 (5 reales sin
> pareja) = **+17.671,25**. Por estrategia: **S6 GANA +3.960,37 · SuperTrend PIERDE −16.483,34**.
> ⚠️ No es veredicto: 2 semanas y 135 posiciones no cruzan la puerta estadística del plan §9. Pero
> **es dato real, no simulado**, y reordena la premisa de «las dos ganadoras hasta ahora».
> 🔴 **BORDE DEL DÍA = LOOK-AHEAD, no un estado no modelado.** `ciclos.py:161` llama
> `ticks.first_at(t)`, definido en `backtest.py:130-139` como búsqueda hacia adelante **SIN COTA**.
> Los 10 casos tienen **cero ticks en su segundo**, así que devuelve el primer tick **después** del
> corte de mantenimiento: **3.592-3.665 s (≈1 h) en nueve casos y 176.699 s (2,04 días) en el de fin
> de semana**. De ahí salen cinco síntomas de un solo defecto: precios del futuro (`:164`), gate de
> spread evaluado sobre **0,50 (futuro)** en vez de **0,60 (vigente, que lo habría RECHAZADO)**
> (`:182`), gate horario sobre el instante pasado (`:176`), `precio_open` del futuro (`:207`) y
> `t_open` sellado con el instante del bucle (`:211`). **El gate no está roto: se le da información
> que no existía.** Misma familia que los fills same-bar ya quemados en `NEGATIVOS.md` (−121 % neto),
> reaparecida en un módulo escrito **después** de aquel veredicto.
> 🔴 **Y ARREGLARLO NO CIERRA EL HUECO.** Descomposición con residuo 1,8 × 10⁻¹²: posiciones
> fantasma **−3.110,14** · reales perdidas **+2.020,72** · **diferencias de precio en las
> emparejadas −4.527,86 ← término MAYOR**. El look-ahead explica el primero y parte del tercero, no
> el grueso.
> 🟢 **Corrección de registro:** donde la prosa decía «137 emparejadas» el artefacto dice **135**
> (`p_cap_resultado.json` + recómputo directo de `comparacion_p_cap.csv`). El 137 lo introdujo el
> controlador y se propagó al estado. Gana el artefacto (§A.9). **Todas las cifras de este bloque
> usan 135.**
> 🟠 **Residuo declarado:** la segunda población de la cola (16 casos `OPEN_SKIPPED_SL_CROSSED`)
> queda **censada y NO explicada**. Y `T0.7-M-A` (precio de cierre contra nivel del stop) **no se ha
> corrido**: su brief está listo en `02-specs/T0.7-M-A-brief-sl-vs-precio-cierre.md`.
> ⏱️ **Presupuesto acordado con el user: el motor se congela al final del 2026-08-16**, y el plan
> entero cierra el **18**. La paridad bit-idéntica **no se alcanza en ese plazo** — decidido
> reportarlo ahora y no el 17.
>
> ---
> **Estado previo (2026-08-13, madrugada del 14), vigente en lo que no contradiga lo anterior:**
...
> 🔴 **TRAMPA, medida el 2026-08-12:** los 4 tests están `slow`-marcados. `pytest
> tests/research/test_baseline_parity.py -q` devuelve **`4 deselected in 0.03s`** — un verde que no
> probó NADA. **Hay que invocarlo con `-m slow`** (`4 passed in 2,01s`). Quien siga la instrucción
> al pie de la letra sin el flag creerá haber validado la paridad sin haberla ejecutado.
...
| `[ ]` | T0.6 | **12 modificaciones de motor** (inventario en plan §4.1) | Spec: Opus · Impl: **Sonnet 5 high · VÍA RÁPIDA (D-47)** | ✅ **B2 RESUELTO (D-22): SIGN-OFF del user.** TODAS juntas, antes del freeze. R1-bis vigente sin excepción. 🔴 **D-47 (2026-08-13, instrucción del user) cambia el protocolo de implementación:** se implementan **lo más rápido posible** con subagentes Sonnet 5 high, **tests breves sólo donde se requieran**, y el grueso del **testeo/depuración se batchea al final**. Se relaja: spec por mod → **spec por grupo** · TDD completo → **smoke test sólo si hay lógica no obvia** · revisión por tarea → **batcheada** · **A6 empírica por mod → una sola A6 al final** (una corrida P-CAP cuesta ~350 s; doce serían >1 h sólo esperando). 🔴 **NO se relaja la puerta de paridad golden tras CADA mod** — cuesta **2,01 s** (`-m slow`, obligatorio: sin el flag da `4 deselected`, verde que no probó nada), o sea **24 s las doce**: nunca fue el cuello de botella, y es lo que hace viable depurar al final, porque sin ella una regresión deja 12 sospechosos y sus interacciones. Es el **cable trampa de R1-bis**. **Triaje** — vía rápida: **1,3,4,5,6,7,8,9** (aditivas); vía normal con spec propia y TDD: **#2** (assert del ratchet), **#10** (overlay entre sustratos), **#11** (camino caliente), **#12** (flujo del replay). **Destino:** el **motor corregido/extendido** que replique cómo operan las estrategias en el mercado real, **validado en ventanas de SOLAPE** (`2026-01`→`2026-08`, único tramo con ticks AVA + Capitaria + posiciones reales), con autorización del user para **descargar los datos que falten** (attach-only, read-only, guard). 🔴 **El motor faulty `b113eb7` NO se corrige**: se replica verbatim y es el patrón de medida. **Riesgo aceptado y declarado:** batchear concentra la depuración al final |
...
| ~~B12~~ | ✅ **CERRADO 2026-08-12.** Barras construidas y verificadas: `data/lake_bars_capitaria/XAUUSD_M15.parquet` (14.358 velas M15 BID, `2026-01-01 20:00` → `2026-08-11 22:15` hora de servidor), derivadas de los ticks por `scripts/research/build_bars_capitaria.py`. **Cobertura verificada por el controlador, no por el agente:** las **152/152** posiciones reales (84 S6 + 68 ST) tienen vela M15 contenedora. **Control de método:** contra las barras nativas del bróker en el tramo común (13.236 velas), `o` coincide **exacto al 100%**; `h`/`l`/`c` difieren en 4-5% de velas con p99 = 0,04-0,05 y solo 9 velas con desvío >0,10 en `h` — la derivación tick→barra reproduce las barras del bróker. 🟠 Residuo declarado: el volumen `v` difiere ~6% sistemático (conteo de ticks del lago vs `tick_volume` nativo de MT5) — **no asumir equivalencia si algún filtro de señal usa volumen**. Hueco diario sistemático 17:00-17:45 server (rollover del bróker); ninguna de las 152 posiciones abre en él. Texto original del bloqueo: ~~**A6 PATA A NO ES EJECUTABLE HOY: no existen barras M15 de Capitaria para la ventana en que operaron las estrategias vivas.**~~ Medido el 2026-08-12 leyendo los dos ficheros, no inferido: `data/lake_ticks/XAUUSD/_bars_M15.parquet` cubre `2026-01-01 20:00` → **`2026-07-24 16:45`**, y la ventana viva de la 902 va de **`2026-07-27 18:53:30`** a `2026-08-11 01:15:04`. **El solape es CERO.** Sin barras M15 no hay señal: S6 y SuperTrend son estrategias M15, así que el motor no puede reproducir ni una sola de las 152 posiciones reales (84 de S6, 68 de SuperTrend). Esto explica por qué A6 nunca se midió: no es que se pospusiera, es que **le faltaba el insumo y nadie lo había comprobado**. 🟢 **Es reparable hoy y con material ya probado:** los ticks de Capitaria SÍ cubren la ventana entera (T0.4 recuperó julio y agosto, +2.584.669 ticks, verificado), y el patrón para derivar barras M15 desde ticks ya existe y está commiteado — `scripts/research/build_bars_ava.py` hace exactamente eso para AVA (fila `F0-DATA-AVA-0008`). Falta su gemelo para Capitaria sobre `2026-07-27`→`2026-08-11`. **Es el desbloqueo de mayor valor del programa**, porque A6 Pata A es lo único que responde a la pregunta "¿el motor es par con lo que pasó de verdad, y con qué fidelidad?" | **A6 Pata A (T0.7) → T0.8 (freeze) → todo veredicto** | **Controlador** |
| ~~B13~~ | ✅ **CERRADO 2026-08-15 — la premisa se cayó, y llevaba tres días caída sin que nadie lo anotara.** La entrega de la máquina 2 llegó el 2026-08-12 y está inventariada en `research/motores/ENTREGA-M2-2026-08-12-inventario.md`: el `repo-completo.bundle` fue **importado y verificado** (tag `engine-faulty-tomachine-902`, commit `b113eb7`), y los **10 ficheros de `config_efectiva/` están verificados byte-idénticos a `b113eb7`**. Esa configuración está además **vendorizada dentro de la réplica** como copia byte a byte del blob de git — `scripts/analysis/realtick_bt/faulty/_vendored_live_configs_20_b113eb7.py`, consumida por `config_faulty.py:36-43` y `llamador.py:330`. Los tres términos del bloqueo quedan satisfechos: **llegó, se verificó, y se está usando**. 🟢 **La discrepancia del volumen que motivó el bloqueo también queda resuelta, por la vía contraria a la temida:** no es que la config viva sea desconocida, es que **es 0,67 lotes** (uniforme y verificado en las 152 posiciones reales) y el `MAX_VOLUME = 0.10` de `reconciler.py:41` es lo que está desalineado respecto al sistema vivo. El 0,67 es el valor que usan `F0-A6-NETO-0001` y todo brief posterior. ⚠️ **Corrección al inventario, detectada al cerrar:** su tabla marca `logs_ejecutor/` (70 MB) como «⏳ sin explotar — es la verdad de terreno de A6», y eso dejó de ser cierto: de ahí salió `data/analysis/p_cap/eventos_ejecutor_902.csv`, que es el insumo de `F0-A6-FILL-0001`, `F0-A6-COLA-0001` y `F0-A6-BORDE-0001`. 🔴 **Lección de proceso, que es lo que hay que llevarse:** este bloqueo afirmaba que «ninguna cifra de A6 Pata A puede declararse veredicto de fidelidad» y bloqueaba `T0.7 → T0.8 → todo veredicto`. Estuvo **resuelto de hecho desde el 12 y abierto en el papel hasta el 15**, invalidando sobre el papel todo lo que se midió en esos tres días. Se anota cuidadosamente cada problema que aparece y se olvida anotar cuándo se resolvió: abrir deja rastro porque molesta, cerrar no lo deja porque alivia. **Texto original del bloqueo, íntegro abajo:** ~~🔴 **La config viva de la 902 NO es verificable desde este equipo, y hay prueba de que difiere de este repo.** Medido el 2026-08-12: las 152 posiciones reales son de **0,67 lotes** (`volume_in` uniforme, verificado por el controlador), pero `sentinel_engine/live/reconciler.py:41` fija `MAX_VOLUME = 0.10` y la ruta de L254-258 **rechazaría la orden** (`volume > MAX_VOLUME`); `CONFIGS_LOCAL` pone 0,1. No existe en este equipo ningún log de decisiones del ejecutor de la 902 — los logs locales son del login `2883015767`. **El plan §4.2 exige replicar «la config viva EXACTA» y hoy eso no se puede satisfacer**, así que ninguna cifra de A6 Pata A puede declararse veredicto de fidelidad. 🔴 **Requiere de la MÁQUINA 2** (donde corre la 902): rama + commit del ejecutor, línea de comandos completa (`--window`, `--roster`, `--max-spread-open`, y lo que module el SL), y el log del ejecutor de la ventana `2026-07-27`→`2026-08-11`. Sin eso, A6 mide el motor de este repo contra un sistema cuya configuración se desconoce~~ | ~~A6 Pata A (T0.7) → T0.8 (freeze) → todo veredicto~~ — ✅ desbloqueado | ~~**User** (aportar desde máquina 2)~~ — entregado 2026-08-12, cierre formal por el **controlador** 2026-08-15 |
...
- **2026-08-12** · Opus 5 (sub-orquestador) + verificación del controlador · 🔴 **LA CAUSA DE LA
  ...
  - **Etapa 0 (capa de señal cruda, sin estado y sin gate):** recall S6 **44,90 %** (+1 barra),
    precisión 47,83 %. Coincidencia real, no artefacto: permutación 2.000 remuestreos, S6 observado
    22 vs media nula 2,71 (**p=0,005**). 🔴 **La capa 1 de ST queda declarada NO EVALUABLE**: su
    autoverificación fue **vacua** (0 de 0 eventos `EXIT_STFLIP`); su 26,19 % no es interpretable.
  - **Sustrato confirmado limpio por dos vías independientes:** barras derivadas ≡ nativas del bróker
    en `o,h,l,c` en las **946/946** velas de la ventana; y el **88,16 %** de los 152 fills reales
    reproduce un tick de `Capitaria-All` exacto (<0,005), con **0 de 303** fills fuera del rango de
    su barra M15. Corrobora el cierre del confusor de servidor.
  - **Confusores (b) y (c) MUERTOS con número:** el gate adaptativo running-min converge a
    **0,500001** y admite **exactamente los mismos 1.817.188 ticks** que la banda fija del harness —
    **son el mismo gate**. Y `active_fichas` ∈ {1,2,3} da **las 12 celdas numéricamente idénticas**.
  - 🔑 **LA CAUSA, medida:** el **SL vivo está 13,1× más cerca** que el del harness. Distancia
    entrada→salida en cierres por SL: mediana **1,34 USD** (S6) / **1,14** (ST); el `init_sl_range_k
    =2.5×rango` del harness da mediana **17,54 USD**. Cadena de consecuencias **verificada por el
    controlador** sobre las 151 posiciones cerradas: duración mediana real **169 s** (S6) / **249 s**
    (ST) con p25 de **18 s** y **41,7 % / 40,3 % cerradas en <60 s**, frente a **6,0 barras (90 min)**
    del motor. La realidad estuvo **plana el 68,1 %** de las barras y pudo re-entrar sin parar
    (84 posiciones desde 49 barras-señal); el motor, ocupado 6 barras de mediana, **se pierde esas
    entradas**. ⇒ **El fallo de paridad de ENTRADAS es consecuencia del fallo de paridad de SALIDAS.**
    La lógica de entrada está sana (dirección 19/19; flips de ST 11/11).
  - **Mod #12 ataca el 7 %:** los cierres manuales son **11 de 151 salidas**; las salidas por **SL son
    118 (78 %)**. Ninguna mod de estado levanta un techo de señal cruda del 45 %.
  - Mecanismo candidato **no medido**: `_clamp_sl` (`run_live_20.py:384-408`), código solo-vivo que
    reposiciona el SL a distancia legal mínima.
  - **Hallazgo lateral:** el gate de spread ya implementa *de facto* el filtro de ventana NY — de 42
    señales S6 descartadas, **41 caen fuera** de 18:00→03:15 NY; en ST, **25 de 25**; y las **49/49 y
    42/42** señales reales caen dentro. Confirma T0.13 desde fuente independiente.
  - 🔴 **BLOQUEO NUEVO (B13): la config viva de la 902 NO es verificable desde este equipo.** Sus
    posiciones son de **0,67 lotes**, pero `sentinel_engine/live/reconciler.py:41` fija
    `MAX_VOLUME=0.10` y **rechazaría la orden**. No hay logs del ejecutor de la 902 aquí. **§4.2 exige
    «la config viva EXACTA» y hoy no se puede satisfacer.** Requiere de la máquina 2: rama+commit,
    línea de comandos (`--window`, `--roster`, `--max-spread-open`) y el log del ejecutor.
  - Contexto: ambas estrategias cerraron la ventana **en positivo** (S6 **+9,27 MM CLP**, ST
    **+5,93 MM CLP**, netos reales de la 902).

- **2026-08-12** · Opus 5 (controlador) · 🔴 **EL CONFUSOR «SERVIDOR DISTINTO» NO EXISTÍA: era la
  razón social leída como servidor.** Medido, no inferido: lanzado `MT5_Tester_2` (`/portable`,
  PID 26800) y attacheado en solo lectura con `path=` explícito, `account_info()` devuelve
  `server='Capitaria-All'`, `company='Capitaria Latam SpA'`.
  - **Origen del error:** la columna de `CUENTAS.md` es `Broker/Server` **combinada**; en la fila de
    la 902 se anotó la **empresa** donde las demás filas llevan el **servidor**. Un informe de A6 lo
    leyó como servidor y dedujo una discordancia («servidores hermanos, no el mismo») que **no
    existe**. `CUENTAS.md` corregido (fila de la 902 + línea de credenciales).
  - **Corroboración independiente:** `MT5_Tester_2` tiene únicamente `Bases/Capitaria-All` —ninguna
    carpeta de otro servidor— y fue el terminal por el que **T0.5** exportó con éxito el historial de
    la 902 el 2026-08-11.
  - ⚠️ **Probado vs inferido, declarado:** *probado* que «Capitaria Latam SpA» es razón social y que
    el servidor de una cuenta Capitaria es `Capitaria-All`; *inferido* (fuerte, no medido directo)
    que la 902 está en `Capitaria-All`. La prueba directa exigiría loguear la 902, cuya clave no se
    persiste (§A.12). No se considera necesaria; si algún diagnóstico dependiera críticamente de
    ello, se escala al user.
  - 🟢 **Consecuencia:** el sustrato de A6 es **válido**, no hace falta re-descargar nada, y **todo
    el residuo de fidelidad pasa a ser atribuible al motor y a la config**.

- **2026-08-12** · Opus 5 (controlador) · ✅ **B12 CERRADO** y **A6 Pata A corrida por primera vez —
  como DIAGNÓSTICO, no como gate.** 🔴 **Se corrió T0.7 sin haber hecho T0.6**, y el plan §4.2 ya
  advertía que sin la **mod #12** (inyección de cierres manuales) A6 *«diverge en cascada tras el
  primer cierre manual»*. Los números de hoy **no son veredicto de fidelidad**.
  - **Dos errores de método detectados por verificación del controlador** (§0.13 funcionando, y
    llegando tarde): (1) el primer informe usó **152 posiciones como denominador**, pero agrupan en
    **49 barras-señal S6 + 42 ST** (hasta 8 posiciones en una misma barra, `mixed_direction=0` ⇒ son
    re-entradas intra-barra de UNA señal); (2) su «precisión 100%» era artefacto de una **tolerancia
    de ±8 barras**. Recalculado a nivel de barra-señal y tolerancia honesta: recall **S6 38,78%
    (tol 0) / 42,86% (±1)**, **ST 45,24% / 47,62%**; precisión 79-95%, no 100%.
  - 🔑 **Hallazgo que reencuadra el déficit:** aplicando el desplazamiento estructural de **+1 barra**
    (la señal cierra en N, el fill real ocurre en N+1), la **dirección coincide 19/19 en ambas
    estrategias, cero errores**; sin desplazar es ruido (S6 0 de 4, ST 9 de 18). El motor **no
    diverge en mecánica: infra-genera**. Perfil compatible con un motor ocupado por posiciones que
    la realidad ya había cerrado a mano ⇒ **mod #12 es la hipótesis dominante**.
  - **A6 Pata B** (Capitaria vs AVA, motor contra motor): comparten **9-13% (S6)** y **0-6,5% (ST)**
    frente al umbral **≥90% ±1** de D-35. ⚠️ Declarado **no definitivo**: falta verificar si la
    «señal cruda» del harness es libre de estado de posición; si no lo es, arrastra la misma cascada.
  - **Confusores vivos tras eliminar el de servidor:** gate de spread **fijo** en el harness vs
    **adaptativo** running-min en el ejecutor real (`run_live_20.py:159,1105-1109`); y en vivo solo
    se observó la ficha **F1** (magics 724011/724071) pese a `active_fichas=3` por defecto.
  - Artefactos: `data/analysis/a6_pata_a/a6_pata_a_signal_level.json` (con emparejamientos
    auditables, para no re-ejecutar), `scripts/analysis/a6_pata_a/signal_level.py`.
  - **En curso:** sub-orquestador Opus despachado — Etapa 0 (capa de señal cruda aislada) → mod #12
    (TDD, paridad D-22 antes y después) → re-correr A6 → cerrar o atribuir con evidencia.
