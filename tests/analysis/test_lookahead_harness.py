...
def _synthetic_ticks(month_data: dict[str, tuple[list[float], list[float], list[float]]]) -> Ticks:
...
def test_first_at_salta_un_hueco_artificial_sin_cota():
    """Hueco de mantenimiento artificial: ticks en 202608 hasta t=100, luego
    nada hasta t=4000 (hueco de 3900s, mayor que cualquier corte de
    mantenimiento medido). Pedir un instante DENTRO del hueco (t=200) debe
    devolver el tick de DESPUES del hueco (t=4000), no None y no un error --
    demuestra que la busqueda no tiene cota temporal, solo la cota de los 3
    meses candidatos."""
...
def test_first_at_cruza_fichero_de_mes_sin_cota():
    """Sin mas ticks en el mes actual tras t_sec, first_at debe tomar el
    PRIMER tick del mes siguiente, sin importar cuan lejos este -- aqui el
    mes siguiente empieza 40 dias despues del ultimo tick del mes actual."""
...
def test_first_at_no_comprueba_distancia_devuelve_precio_tal_cual():
    """first_at no aplica ninguna tolerancia: el precio del tick lejano se
    devuelve identico al almacenado, sin marca de que sea "lejano"."""
    base = float(int(datetime(2026, 8, 1, tzinfo=timezone.utc).timestamp()))
