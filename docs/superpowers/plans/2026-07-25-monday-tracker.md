...
| 11 | Ensayo de revert | B | Sonnet 5 high | `[ ]` | Puede correr en paralelo con R1/R2 pero NO simultáneo con commits de otros (muta estado git) |
...
|---|---|---|---|---|
| R1 | Pairing "reverse" en `run_ladder` | Sonnet 5 high | `[ ]` | TDD; caracterizar el evento ANTES de implementar |
| R2 | Jerarquía de fills + `fill_source` + asserts | Sonnet 5 high | `[ ]` | 4 caminos cerrados en el plan; NO hay `_bars_M1.parquet` en el lake |
| R3 | Regenerar + re-correr Track A + diff | Sonnet 5 high | `[ ]` | snapshot previo OBLIGATORIO; multiset de las 2.347 filas viejas debe sobrevivir |
