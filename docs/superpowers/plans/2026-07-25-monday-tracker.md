...
### 🔴 SUSTITUCIÓN 2026-07-27 (Task R4) — valores del substrato REPARADO

> **Registro aditivo.** La tabla de arriba se conserva intacta a propósito: documenta qué se creyó
> y por qué. **Todas sus filas quedan SUPERADAS por esta tabla**, salvo la de cobertura del
> calendario B2, que no cambia. Motivo único de la sustitución: el substrato sobre el que se
> calcularon aquellas cifras **no contenía los cierres por stop-and-reverse** (bug de emparejamiento
> de `run_ladder`, corregido en R1 `fadf1ec`; substrato regenerado en R3 `4956d60`). Las 2.347 filas
> viejas sobrevivieron con `net` idéntico al céntimo; el delta completo son **195 filas nuevas**.
> Veredictos completos: `docs/superpowers/research/2026-07-27-substrate-repair-diff.md` §VEREDICTOS.

| Dato | Valor VIGENTE | Sustituye a | Por qué / veredicto R4 |
|---|---|---|---|
| `max_open_fichas` (cap B3) | **7** (`a2_overlap.json`, 2.542 posiciones) | 7 (mismo valor) | **SE SOSTIENE.** El histograma de simultaneidad subió en todos los bins y el pico sigue en 7. 🟢 **`CONFIGS_CHALLENGER` (726xxx) NO requiere cambio; no se tocó.** Task 12 queda liberada de este condicionante. Matiz: `dropped_n=0` por construcción → es un **fusible**, no un control de riesgo validado |
| Correlación diaria de neto por par | **S6\|S7 +0,767 · S6\|ST +0,356 · S7\|ST +0,153** sobre **163** días | +0,760 / +0,352 / +0,141 (pre-reparación) | **SE SOSTIENE** (movimiento en el 3.er decimal). La frase de Task 8 «SuperTrend es casi independiente» **sigue enterrada**. 🔴 Nuevo: ST aporta el **53 %** del neto reparado con 268 posiciones — la diversificación del track descansa sobre una sola pata |
| maxDD @0.1 lot | **−32,34 %** del balance inicial = **−19.276.260 CLP**; −25,07 % del pico de equity; `trough_equity` **−1.994.745** (P&L acumulado bajo su punto de partida; saldo 76,9 MM → 57,6 MM, **la cuenta NO se agotó**) | −28,57 % / −17.028.660 CLP | 🔴 **CAMBIA. La frase «CONFIRMA la estimación previa de −28,6 %» queda MUERTA** y no puede repetirse. La *coherencia interna* sí sobrevive: 129.150.945 × 0,1/0,67 = 19.276.260,45 ≈ 19.276.260,47 del artefacto. `t_peak`/`t_trough` idénticos |
| **Neto COMBINED 7 meses (0.67 lot)** | **39.513.978,56 CLP** · n=2.542 · WR 32,26 % · **PF 1,033** | 147.780.084,05 · n=2.347 · PF 1,138 | 🔴 **CAMBIA (−73,26 %).** No es un deterioro del sistema: es el instrumento contando por fin los cierres `reverse`. S6 −80,93 % · S7 −97,98 % (queda en 670.122, PF 1,0013) · **SuperTrend idéntico (20.983.977,53)** |
| Veredictos de máscara B1–B4 | **B1 NO-VETO** (227 vetadas, **+40.788.967 CLP**: 39,5 → 80,3 MM) · **B2** 0 descartes · **B3** 0 descartes · **B4** NO EVALUABLE | «_pendiente_» (Task 10) | **Los 4 veredictos SE SOSTIENEN**; cambian las magnitudes. ⚠️ El `net_delta_pct` de B1 pasa a **+103,23 %** pero está **inflado por la caída de la base**: en CLP la mejora subió 38 %, no 400 %. Citar en CLP |
| 🆕 Atribución por motivo de salida | `EXIT_SL_RAISED` 1.980 → +129,1 MM · `EXIT_STLINE` 268 → +21,0 MM · `EXIT_TRAIL` 84 → +12,6 MM · **`reverse` 195 → −108,3 MM** · `EXIT_INITSL` genuino **15**, **WR 0 %** → −14,9 MM | «EXIT_INITSL n=891/1104, rentable» (Task 9) | 🔴 **CAMBIA. Corrección narrativa nº 1.** El track **gana con el trailing y pierde con las reversiones**. El stop inicial genuino se toca en el **0,6 %** de los cierres |
| 🆕 Concentración del neto (D171) | Quitar **3** ops (mayores \|net\|) → **−7.557.696**; quitar 10 → −66.525.916. Por estrategia: quitar **1 de cada una (3 ops)** → **+1.052.242** (2,7 % del neto); 30 ops → −157.907.835 | «10 ops = 72 % del neto; 30 lo vuelven −49,6 MM» | 🔴 **CAMBIA y empeora.** Las sumas top-K son **idénticas al céntimo** en ambos substratos: la cola no cambió, se encogió el cuerpo. **Bajo D170 el track NO supera el criterio de recorte top-K** |
| 🆕 Completitud del substrato | De 6.417 emparejamientos crudos (S6+S7) llegan al libro **2.274 (35,4 %)**; `reverse` sobrevive **26,2 %** (banda de `EXIT_TRAIL` 28,3 %); **549 cierres `reverse` siguen fuera** por el gate de spread 0,5 (espejo del vivo) + ventana de reintento | — (nunca medido) | **INDETERMINADO acotado.** Lo que R1 arregló está arreglado; lo que falta, falta por el gate, no por el emparejador. Causa medida = **duración corta** de los reverse, no su motivo. **39,5 MM es la mejor cifra disponible, no una cifra cerrada**; dirección del sesgo residual **no determinable** hoy. Medición de R4 por código, **no persistida en artefacto** |
...
| 12 | Despliegue máq1 | B | Sonnet 5 high | `[!]` **CONGELADA** | 🙋 **pasos 5-6 = manos del user**. Recomendado tras R4 (el cap B3=7 se calibró sobre substrato incompleto y podría moverse); timing = decisión del user |
| 13 | Documento del lunes | — | **Opus 5 high** | `[x]` | 🟢 **DESGATEADA 2026-07-27: R3 y R4 completos.** ~~CUATRO correcciones narrativas obligatorias: corr ST ya no "casi independiente" (0,35); grilla mensual regenerada; reporte mensual sin snapshot previo; 🆕 disclosure bug reverse+fills con números del re-run~~ → **SUPERADAS por la lista de 17 de R4.** Estado de las cuatro originales tras revisión contra el dato nuevo: (1) corr ST → **VIGENTE**, número actualizado a 0,356/0,153; (2) grilla mensual → **VIGENTE**, fuente actualizada al re-run post-reparación; (3) «reporte mensual sin snapshot previo» → 🔴 **OBSOLETA, RETIRAR**: R3a (`b3c53e2`) creó el snapshot y el diff viejo-contra-nuevo **existe**; (4) «bug reverse+**fills**» → 🔴 **REESCRIBIR**: hay UN bug (reverse) y UNA etiqueta engañosa (`EXIT_INITSL`); **el bug de fills NO existe** (refutado: 891/891 y 1104/1104 cruzan). 📋 **Lista completa y accionable: §V17 del diff doc, 17 puntos.** El nº 1 (cómo se comunica la caída del neto) y el nº 8 (concentración D171) son los críticos |
...
| R4 | Veredictos sobre el diff | **Opus 5 high** | `[x]` | commit `92f0b65` (§Veredictos) + commit de este tracker. §VEREDICTOS (V0–V17) añadida al final de `docs/superpowers/research/2026-07-27-substrate-repair-diff.md`: 11 bloques dictaminados SE SOSTIENE / CAMBIA / INDETERMINADO + lista de **17 correcciones obligatorias** para Task 13 (§V17). **Cap B3 NO se movió** (sigue 7): el roster retador no se tocó, ningún `.py` modificado. 🔴 Tres contradicciones registradas (§V15): el bug de fills no existía; la corrección del controlador sobre re-atribución era errónea (0 re-atribuidas — la expectativa original del plan era la correcta); el «CONFIRMA −28,6 %» de Task 7 cae |
...
- 2026-07-27 · Task R4 · **Veredictos sobre el diff del substrato reparado.** §VEREDICTOS (V0–V17) añadida al final de `docs/superpowers/research/2026-07-27-substrate-repair-diff.md`, sin tocar ni una cifra de las 11 secciones de R3 y sin modificar ningún `.py`. **Dictamen central:** las estrategias vivas NO empeoraron — las 2.347 filas viejas conservan su `net` **al céntimo** (147.780.084,05 sumado directamente sobre las CSV nuevas) y el delta completo son 195 filas nuevas, 100 % `reason="reverse"`, por −108.266.105,49. **CAMBIAN:** neto (147,8 → 39,5 MM, −73,26 %; PF 1,138 → 1,033), maxDD (−28,57 % → **−32,34 %**, y el «CONFIRMA» de Task 7 queda muerto), atribución A3 (el track gana con el trailing y pierde con las reversiones; stop inicial genuino = 15 cierres con WR 0 %), concentración D171 (**bastan 3 operaciones para volver negativos los 7 meses**), curva B1 (los 5 peldaños ahora positivos; «el signo se da vuelta entre 60 y 90 min» ya no es cierto; N2↔N5 intercambian puesto; el ranking se rompe en K=10), B1 magnitud (+20,01 % → +103,23 %, **inflado por la caída de la base**). **SE SOSTIENEN:** cap B3 = 7 (🟢 **roster retador `CONFIGS_CHALLENGER` NO requiere cambio y no se tocó**; Task 12 liberada de ese condicionante), correlaciones (3.er decimal), A4 serial, A5 no evaluable, los 4 veredictos de máscara, perfil de signo mensual (mismos 5+/2−), M3 bloqueadas-vs-mantenidas, y la validación de la semana vivida (Δ=0 en las 8 celdas — pero R4 midió que esa semana tiene **0 cierres `reverse`**, así que es control de no-regresión, no confirmación). **INDETERMINADO nuevo (V14):** R4 midió con código read-only que de 6.417 emparejamientos crudos S6+S7 solo 2.274 (35,4 %) llegan al libro y que `reverse` sobrevive al 26,2 % — banda de `EXIT_TRAIL` (28,3 %), causa medida = duración corta (mediana 3-4 barras vs 4-7), **no** discriminación por motivo; 549 cierres `reverse` siguen fuera por el gate de spread 0,5, que espeja el vivo. 39,5 MM es la mejor cifra disponible, **no una cifra cerrada**. 🔴 **Contradicciones registradas (§V15), el dato gana en las tres:** (a) el bug de fills del plan **no existía**; (b) la corrección del CONTROLADOR sobre re-atribución de filas era **equivocada** — 0 re-atribuidas, 0 desaparecidas, es decir **la expectativa original del plan era la correcta** y su gate pass-fail habría pasado (mecanismo: la entrada nueva sobreescribía la vieja en `open_pos`, así que la fila emitida era correcta y la pierna del reverse desaparecía entera); (c) tres afirmaciones commiteadas del tracker quedan superadas (Task 7 maxDD, Task 16 signo N6, Task 9 A3). §"Números que otras tareas necesitan" sustituida en registro **aditivo** (tabla vieja intacta + tabla nueva con qué sustituye a qué y por qué). Lista de **17 correcciones obligatorias para Task 13** en §V17. ℹ️ Observación al controlador, no tocada por no ser sección propia: **R3 sigue marcada `[~]`** en la tabla de la §EXTENSIÓN pese a estar cerrada con commit `4956d60`. · commit `92f0b65` (§Veredictos), commit de este tracker a continuación
- 2026-07-27 · Task R2-bis · `EXIT_INITSL` separado en `EXIT_INITSL` (stop inicial genuino) y `EXIT_SL_RAISED` (stop ya levantado por trailing/BE y tocado despues), en la capa de analisis. `_sl_inicial` resulto **NO importable** (esta anidada dentro de `simular_variant` y cierra sobre `bars`/`init_sl_range_k` locales), asi que se reimplemento como `_sl_inicial_genuine` en `backtest.py` **citando verbatim `emasar_variant.py:621-624` en el docstring** para que la deriva sea detectable — el camino de respaldo que el brief ya preveia. `k` se lee de los kwargs vivos (`_GL[sid]["init_sl_range_k"]`), no hardcodeado; hay test que lo prueba con dos `k` distintos. TDD: rojo antes, 6/6 verde despues; `tests/analysis -q` **67 passed** (61 + 6), cero regresiones. Conteo in-memory: S6-K2P0 12 genuinas / 879 levantadas, S7-TPNONE 3 / 1101 — **coincide exacto con lo medido por R2 en H3**, incluida la particion de rentabilidad. Sensibilidad: con `TOL=1e-3` en vez de `1e-6` cambian **0** filas de grupo en ambas estrategias. 🔴 **Desviacion declarada y NECESARIA:** hubo que anadir `"EXIT_SL_RAISED"` a `LEVEL_EXITS`, porque `resolve()` branchea sobre `reason in LEVEL_EXITS` para decidir entre fill por cruce de tick intra-barra y fill a cierre de barra; omitirlo habria corrompido en silencio el **precio de salida** (no solo la etiqueta) de ~99 % de las filas reclasificadas. Verificado en el fichero: `LEVEL_EXITS = {"EXIT_INITSL", "EXIT_SL_RAISED", "EXIT_TRAIL", "EXIT_TP", "EXIT_STLINE"}`. Inventario de consumidores: el unico afectado de forma relevante es `a3_a4_a5_stats.py` (A3), que es agnostico al string y se auto-dividira en dos grupos al regenerar — sin cambio de codigo. Duda abierta para R4: si `EXIT_SL_RAISED` es el nombre definitivo que el programa quiere aguas abajo. · commit `2823160`
- 2026-07-27 · Task 13 · **Documento EMITIDO** (no "entregado": eso requiere revision del user): `docs/INFORME_CAMPEON_RETADOR_2026-07-27.md`, 881 lineas, escrito contra el substrato REPARADO y contra la §Veredictos de R4. Las 17 correcciones obligatorias de §V17 cumplidas 17/17, cada una con su seccion (checklist en el reporte de tarea). Afirmaciones centrales: neto 7 meses **39.513.979 CLP** (PF 1,033, WR 32,26 %, 2.542 ops); los 147,8 MM **nunca fueron el resultado del sistema** sino el de un sistema que no pagaba sus reversiones, con la prueba junto al titular (las 2.347 posiciones ya mostradas conservan su resultado al centimo, 0 re-atribuidas, 0 desaparecidas; el delta completo son 195 cierres `reverse` reales); el titular honesto es el **PF 1,138 -> 1,033**; SuperTrend aporta el **53 %** del neto y es la unica pata poco correlacionada, o sea la diversificacion descansa sobre una sola estrategia; **TRES operaciones vuelven negativos los 7 meses** (seccion propia, la segunda del documento, y bajo D170 el track NO supera el criterio de recorte top-K); **39,5 MM es la mejor cifra disponible, no una cifra cerrada** (549 cierres `reverse` siguen fuera por el gate de spread 0,5, sesgo residual declarado NO EVALUABLE); el track gana con el trailing (`EXIT_SL_RAISED` 1.980 -> +129,1 MM) y pierde con las reversiones (195 -> −108,3 MM). Divulga el proceso, incluido que **una de las dos hipotesis iniciales (el bug de fills) resulto FALSA al medirla**, asi que el documento no la afirma. Dos advertencias operativas explicitas: el retador **NO esta desplegado** (Task 12 pendiente, manos del user) y el **tamano de muestra minimo para que el A/B signifique algo se declaro NO DISPONIBLE** (nadie corrio un calculo de potencia; estimarlo habria violado R5). Cero `.py`, cero artefactos de datos, cero configs tocados. · commit `286c3c7`
- 2026-07-27 · Revision de codigo BATCHEADA de fin de fase (Opus 5 high, doctrina: batch a cierre de fase, no por tarea) · **APROBADO CON OBSERVACIONES: 0 Critical, 3 Important, 4 Minor.** Revision en `.superpowers/sdd/2026-07-27-substrate-repair-and-program-extension/phase1-code-review.md`. **Validacion independiente que sostiene el aprobado:** el revisor re-emparejo los eventos del motor con un algoritmo ALTERNATIVO sobre los 7 meses reales y obtuvo **0 discrepancias** campo a campo en 2.964 filas (S6) y 3.453 (S7), `reason` incluida; y la aritmetica cierra exacta: `39.513.979 + 108.266.105 = 147.780.084`, es decir R1 explica el 100 % de la caida y R2-bis movio cero pesos, justo lo que debe hacer un re-etiquetado puro. Focos verificados correctos: el `pop` por ficha en el caso reverse+entrada-misma-barra, y cero conversiones de zona horaria introducidas. Sin desincronizacion en `scripts/analysis/**` (el `reason` se consume solo como clave de agrupacion dinamica). Los 3 Important son de FRAGILIDAD, no de numero mal calculado: (1) la pertenencia de `EXIT_SL_RAISED` a `LEVEL_EXITS` vale ~−83,6 MM y **ningun test la protege** — borrarla deja el 87 % de las filas con fill a cierre de barra y el neto pasa a ~−44 MM con la suite EN VERDE; (2) `_sl_inicial_genuine` reimplementa el helper equivocado (el motor llama `_sl_inicial_bounded`, no `_sl_inicial`; identicos solo mientras `wait_mae_atr_k == 0.0`, palanca que **la matriz de experimentos del programa post-lunes barre**); (3) ningun test discrimina la barra de entrada (fixture de barras uniformes), asi que mutar `pos["idx"]`->`ev["idx"]` deja 12 tests verdes e invierte 33/33 filas de S6 y 12/12 de S7 sobre datos reales. Los 4 Minor quedan diferidos y registrados. · sin commit (revision)
- 2026-07-27 · Correccion de los 3 Important de la revision de fase (TDD, los tres rojos pegados y revertidos antes del verde final) · `tests/analysis` **67 -> 71 passed** (4 tests nuevos). 🔴 **Pregunta critica de la tarea, respondida: NINGUN numero del substrato cambio.** `wait_mae_atr_k` esta AUSENTE (default 0.0) en los kwargs vivos de S6-K2P0 y S7-TPNONE, asi que la correccion del helper es un no-op byte-identico: conteos 12/879 (S6) y 3/1101 (S7) identicos antes y despues, neto combinado identico antes y despues. No hubo que escalar, y el documento del lunes NO requiere revision de cifras. Desglose: I-1 (pertenencia de `EXIT_SL_RAISED` a `LEVEL_EXITS`, que vale ~−83,6 MM y no tenia test) y I-3 (ningun test discriminaba la barra de entrada) fueron **blindaje por tests puro**, sin cambio de codigo de produccion — el comportamiento ya era correcto, lo que faltaba era la red que impide romperlo en silencio. I-2 SI requirio corregir `_sl_inicial_genuine`/`run_ladder` en `backtest.py` para alinearlos con `_sl_inicial_bounded`, que es el helper que el motor realmente llama; `sentinel_engine/**` solo se leyo, jamas se modifico. La trampa que cierra I-2 es de futuro, no de presente: la matriz de experimentos del programa post-lunes **barre `wait_mae_atr_k`**, y con esa palanca > 0 el 100 % de los stops iniciales genuinos se habria etiquetado mal sin fallar ningun test ni mover el neto. Los 4 Minor quedan diferidos e intactos. Cero regeneracion de CSV, cero cambios en `docs/**` y `data/**`. · commit `4bb0fc4`

## 🟢 CIERRE DE LA ENTREGA DEL LUNES — 2026-07-27

**15 de 16 tareas del lunes cerradas.** La unica pendiente es **Task 12 (despliegue en maquina 1),
que son manos del user por diseno** — nada se despliega sin su decision explicita. Tasks 14 y 15
siguen abiertas y NO bloquean el lunes (ambas deben correr sobre el substrato reparado).

Cadena de reparacion completa: R1 `[x]` · R2 `[!]` cerrada sin implementar por refutacion medida ·
R2-bis `[x]` · R3a `[x]` · R3 `[x]` · R4 `[x]` · revision de fase APROBADA CON OBSERVACIONES ·
3 Important corregidos y blindados por tests · Task 13 EMITIDA.

**Estado de tests:** `tests/analysis` 71 passed (baseline de inicio de sesion: 57).
**Substrato vigente:** 2.542 posiciones, neto 39.513.979 CLP, PF 1,033. El "antes" congelado e
inmutable en `data/analysis/pre_repair_snapshot/`.

## 🛑 CONGELACION DE DESPLIEGUE — decision del user 2026-07-27

**NO se despliega NADA en vivo hasta nuevo aviso explicito del user.** Task 12 (despliegue del
sleeve retador 726xxx en maquina 1, DEMO 2883015767) queda **CONGELADA**, no pendiente.

Motivo del user, verbatim en sustancia: primero el backtest largo con todos los candidatos de neto
positivo corriendo junto a las tres estrategias vivas actuales; solo despues, y con esos numeros
delante, se decide si existe siquiera un retador relevante que merezca desplegarse.

**Consecuencias vinculantes para cualquier sesion futura:**
- Ninguna tarea puede iniciar un despliegue, arrancar un supervisor, ni modificar el roster vivo.
- `CONFIGS_CHALLENGER` se queda EN EL CODIGO pero SIN desplegar: existe, esta testeado, no corre.
