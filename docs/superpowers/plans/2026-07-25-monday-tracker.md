...
| R2 | Jerarquía de fills + `fill_source` + asserts | Sonnet 5 high | `[~]` | 🔴 **Spec MODIFICADA por medición del controlador** (ver §Hallazgos H1/H2 abajo): la rama "hueco de cobertura → fallback M1" del plan **no puede dispararse nunca** (0 ventanas vacías de 13.236) → sería código muerto, NO se implementa. Vocabulario real de `fill_source`: `tick` \| `level_uncrossed` (+ `bar_close` para salidas que no son de nivel). Además la CAUSA del bug NO está establecida → la tarea lleva un Step 1 de diagnóstico con escalada obligatoria si aparece un tercer bug |
