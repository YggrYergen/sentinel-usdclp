"""
sentinel_engine.technical — instrument-parameterized TechnicalScorer (P1, Task 1.5).

Reproduces the EXACT math of `sentinel.technical_scorer.calculate_multi_tf_score`
(and its `calculate_technical_score` / `_score_*` / `detect_rsi_divergences`
helpers) but sources the timeframe set, bar count, timeframe weights, and
target symbol from an `sentinel_engine.config.InstrumentConfig` instead of
module-level USDCLP-hardwired globals (`sentinel.config.TIMEFRAMES`,
`BARS_TO_FETCH`, `sentinel.technical_scorer.TF_WEIGHTS`).

Reuses `sentinel.indicators.calculate_all` / `get_latest_signals` verbatim —
the indicator math itself is NOT reimplemented here, since it is driven by
the global `sentinel.config.INDICATORS`, identical for every instrument.

This module is purely additive: it does not modify or import from
`sentinel.technical_scorer`.
"""
...
# (not per-instrument) — identical to sentinel.technical_scorer's inline
# `w = {...}` dict.
...
def detect_rsi_divergences(tf_scores: dict) -> list:
    """Byte-identical reproduction of
    `sentinel.technical_scorer.detect_rsi_divergences`."""
    divergences = []
...
class TechnicalScorer:
    """Multi-timeframe technical scorer parameterized by an `InstrumentConfig`.

    Usage mirrors `sentinel.technical_scorer.calculate_multi_tf_score`:
        ts = TechnicalScorer(load_instrument("gold"))
        result = ts.score(feed)
    """

    def __init__(self, cfg: InstrumentConfig):
        self.cfg = cfg

    def score(self, feed: Feed, symbol: str | None = None) -> dict:
        """Byte-identical reproduction of
        `sentinel.technical_scorer.calculate_multi_tf_score`, sourcing
        timeframes/bars/tf_weights from `self.cfg` instead of
        `sentinel.config.TIMEFRAMES` / `BARS_TO_FETCH` / module-level
        `TF_WEIGHTS`."""
        if symbol is None:
            symbol = self.cfg.target

        tf_weights = self.cfg.technical.tf_weights
        bars = self.cfg.data.bars_to_fetch

        tf_scores = {}
        for tf_name, tf_min in self.cfg.data.timeframes.items():
